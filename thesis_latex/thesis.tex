...
The Z-shaped membership function implements a falling linear ramp. It assigns a high membership degree when the input is below a starting threshold ($A$) and a low membership degree when it exceeds the end threshold ($B$). For intermediate values, it computes the declining slope using the optimized lookup dividing logic. The SystemVerilog code is provided in Listing \ref{code:mf_z_dig}.
